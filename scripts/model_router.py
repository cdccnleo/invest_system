...
WHITELIST_RULES = [
    ...
    # ── 强制 DeepSeek（不可降级）────────────────────────────────
    ("strategy", "generate", r"策略|操作计划|建议(?!.*查)|买卖|增持|减持|调仓|建仓|清仓|加仓|减仓|换仓|仓位管理",  # noqa: E501
     "deepseek", "策略生成"),
    ("macro", "analysis", r"宏观|政策|利率|GDP|CPI|美联储|央行|货币|财政|经济形势|基本面.*分析",
     "deepseek", "宏观分析"),
    ("sector", "analysis", r"行业|板块|赛道|轮动|景气|周期|产业|产业链",
     "deepseek", "行业分析"),
    ("stock", "research", r"基本面|估值|PE(?!.*多少)|PB|ROE|净利润|营收|毛利率|竞争优势|财务.*分析|价值.*投资",  # noqa: E501
     "deepseek", "个股基本面"),
    ("judge", "action", r"能不能买|该不该卖|要不要|可以加仓吗|帮我决定|要不要清仓|要不要止损|要不要换|哪个好|风险评估|帮我评估|请评估|评估一下",  # noqa: E501
     "deepseek", "决策判断"),

    # ── Ollama 本地查询（中文词更宽泛匹配）─────────────────────────
    ("query", "holding", r"持仓|仓位|成本|盈亏|市值|份额|资金|现金|我的.*股|股票.*收益|基金.*收益",
     "ollama", "持仓查询"),
    ("query", "price", r"现价|当前价格|行情|报价|收盘|开盘|涨跌|涨了多少|跌了多少|价格.*多少|PE.*多少|股价",  # noqa: E501
     "ollama", "行情查询"),
    ("calc", "technical", r"RSI|MACD|布林带|均线|KDJ|ATR|OBV|威廉|斐波那契|技术指标",
     "ollama", "技术指标计算"),
    ("calc", "general", r"计算|收益率|年化|夏普比率|最大回撤|胜率|盈亏比",
     "ollama", "通用计算"),
    ("news", "summary", r"新闻|快讯|今日要闻|市场消息|涨了|跌了|为什么涨|为什么跌|利好|利空|大盘.*如何|大盘.*表现|整体.*如何",  # noqa: E501
     "ollama", "新闻/行情解释"),
